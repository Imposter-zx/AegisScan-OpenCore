...
class EngagementMemory:
    ...
    def record_block(self, technique_id, details=""):
        self.logger.warning(
            f"[MEMORY] Technique {technique_id} was blocked/slowed. Recording for avoidance."
        )
        self.blocked_techniques.add(technique_id)

    def record_success(self, evasion_id):
        self.logger.info(
            f"[MEMORY] Evasion pattern {evasion_id} was successful. Biasing future decisions."
        )
    ...
    def log_behavior(self, latency, status_code):
        self.target_behavior_history.append(
    ...
    def get_session_pressure(self):
        ...
        errors = [
            h["status_code"]
            for h in self.target_behavior_history
            if h["status_code"] in [403, 429]
        ]

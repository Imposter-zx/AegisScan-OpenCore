...
class MissionEngine:
    ...
    def record_action(self, noise_cost=5, intel_gain=10):
        ...
        self.logger.info(
            f"[MISSION] Budget Update: Noise {self.current_noise}/{self.noise_budget} | Intel {self.intel_value}/Goal"
        )
    ...
    def should_disengage(self):
        ...
        if self.current_noise >= self.noise_budget:
            self.logger.critical(
                "[STRATEGIC] Noise budget EXCEEDED. Initiating tactical disengagement."
            )
            return True
        return False
    ...
    def evaluate_gain_vs_exposure(self):
        ...
        if self.intel_value > 0 and self.current_noise > 0:
            ...
            if ratio < 0.5 and self.mission_type == MissionTypes.RECON:
                self.logger.warning(
                    "[STRATEGIC] High Exposure/Low Gain detected. Recommending restraint."
                )
                return "CAUTION"
        return "STABLE"

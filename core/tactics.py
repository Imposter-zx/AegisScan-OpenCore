    ...
    def decide_next_phase(self, last_results=None):
        ...
        if pressure > 7:
            self.logger.critical(
                "[TACTICS] High defensive pressure detected. Force transitioning to DISENGAGE."
            )
            self.current_state = TacticalStates.DISENGAGE
            return self.current_state
        ...
        if self.mission:
            ...
            if health == "CAUTION":
                self.logger.warning(
                    "[STRATEGIC] Strategic caution applied. Avoiding aggressive phases."
                )
        ...
        if pressure > 4:
            self.logger.warning(
                "[TACTICS] Moderate pressure. Moving to PRESSURE_DETECTED protocols."
            )
            self.current_state = TacticalStates.PRESSURE_DETECTED
            return self.current_state
        ...
        if self.current_state == TacticalStates.RECON:
            ...
            if last_results and any(
                f.get("severity") in ["HIGH", "CRITICAL"] for f in last_results
            ):
                ...
                if self.mission and self.mission.intel_value > 20:
                    ...
                    self.logger.info(
                        "[STRATEGIC] Insufficient intel value to justify active validation. Staying in Enumeration."
                    )

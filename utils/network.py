...
def is_packet_capture_feasible(target):
    ...
    if not is_local_network(target):
        logger.warning(
            f"Target {target} is NOT on a local/private network. Skipping traffic analysis for safety."
        )
        return False
    ...
    try:
        subprocess.run(
            ["tshark", "-v"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True
        )
    ...
    if os.name == "nt":
        try:
            ...
            if ctypes.windll.shell32.IsUserAnAdmin() == 0:
                logger.warning(
                    "Packet capture requires Administrative privileges on Windows."
                )
                return False
        except:
            return False
